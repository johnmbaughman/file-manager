    ...
    public class FileManager
    {
        ...
        public void ChangeSection()
        {
            if (Esection == ESection.Right)
            {
                ...
                Console.Write($"Current path: {GUI_Orig.GetNormalString(LPath, Config.HalfWindowWidth - 19)}");

                // переписать текст справа
                Console.BackgroundColor = Config.BackgroundColor;
                Console.SetCursorPosition(Config.HalfWindowWidth + 2, 3);
                Console.Write($"Current path: {GUI_Orig.GetNormalString(RPath, Config.HalfWindowWidth - 19)}");
        ...
        public void Reload(bool RefreshFilesList = true)
        {
            ...
            GUI_Orig.DrawMenu(FileManagerName);
